package rlbotexample.bot_behaviour.panbot.implementation.normal_1s.normal_1s_v4.state.main_behaviour_state.ground_dribble_state;

import rlbot.render.Renderer;
import rlbotexample.bot_behaviour.panbot.BotBehaviour;
import rlbotexample.bot_behaviour.panbot.implementation.normal_1s.normal_1s_v4.state.main_behaviour_state.FiveHeadAerialState;
...
public class AlignCarForShwoooPakState implements State {
    ...
    public void exec(DataPacket input) {
        ...
        rightTurn = GroundDrivingTrajectoryFinder.getRightTurningTrajectory(input.car);
        leftTurn = GroundDrivingTrajectoryFinder.getLeftTurningTrajectory(input.car);
        ...
        if(input.ball.velocity.minusAngle(destination.minus(input.ball.position)).y > 0) {
            closestPositionOfCar = rightTurn.compute(timeToCheck);
        }
        else {
            closestPositionOfCar = leftTurn.compute(timeToCheck);
        ...
        Vector3 groundOrientation = input.car.orientation.noseVector
                .plusAngle(closestPositionOfBall.minus(input.car.position)
                        .minusAngle(closestPositionOfCar.minus(input.car.position)));
