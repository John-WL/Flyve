...
public class RefuelProximityBoost extends SkillController {
    ...
    public RefuelProximityBoost(BotBehaviour bot) {
        this.bot = bot;
        this.driveToDestination = new DriveToDestination(bot);
    }
    ...
    public void updateOutput(DataPacket input) {
        ...
        for(BoostPad pad: notTakenBoostPads) {
            if(pad.getLocation().minus(playerPosition).magnitude() < 700
                    && Math.abs(playerNoseOrientation.flatten().correctionAngle(pad.getLocation().flatten())) < Math.PI/6) {
