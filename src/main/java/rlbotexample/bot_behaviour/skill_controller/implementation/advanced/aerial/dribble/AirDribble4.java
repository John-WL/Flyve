...
public class AirDribble4 extends SkillController {
    ...
    private double distanceFrontBackCoef = -0.1;
    private double velocityFrontBackCoef = -0.1;
    private double distanceLeftRightCoef = -0.1;
    private double velocityLeftRightCoef = -0.1;
    ...
    public AirDribble4(BotBehaviour bot) {
    ...
    public void updateOutput(DataPacket input) {
        ...
        aerialOrientationHandler.setRollOrientation(
                smoothedOutBallDestination.minus(input.allCars.get(1-input.playerIndex).position).normalized()
                .plus(input.allCars.get(1-input.playerIndex).velocity.normalized())
        ...
        boolean isBoosting = boostController.process(
                Math.max(
                        smoothedOutBallDestination.minus(input.allCars.get(1-input.playerIndex).position).z
                                + input.allCars.get(1-input.playerIndex).position.minus(input.car.position).z / 0.75
                                - input.allCars.get(1-input.playerIndex).velocity.z/1.5,
                        50));
    ...
    private Vector3 findOptimalDeltaPositionOnCar(DataPacket input) {
        Vector3 desiredBallVelocityTowardsDestination = ballDestination.minus(input.allCars.get(1-input.playerIndex).position).scaledToMagnitude(ballTargetSpeed);
        Vector3 extraBallVelocity = input.allCars.get(1-input.playerIndex).velocity.minus(desiredBallVelocityTowardsDestination);
    ...
    private Vector3 findNeutralNoseDestination(DataPacket input) {
        ...
        Vector3 noseOrientation = input.allCars.get(1-input.playerIndex).position
    ...
    private Vector3 findOptimalRollOrientationDestination(DataPacket input) {
        ...
        //return input.allCars.get(1-input.playerIndex).position;
    }

    private Vector3 getPositionFromBall(DataPacket input) {
        return input.car.position.minus(input.allCars.get(1-input.playerIndex).position);
    }

    private Vector3 getVelocityFromBall(DataPacket input) {
        return input.car.velocity.minus(input.allCars.get(1-input.playerIndex).velocity);
    }
