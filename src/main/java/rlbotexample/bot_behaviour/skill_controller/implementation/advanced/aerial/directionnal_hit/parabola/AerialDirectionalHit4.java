...
public class AerialDirectionalHit4 extends SkillController {
    ...
    private void updateCarPredictedTrajectory(Vector3 naiveOrientationGuess, DataPacket input) {
        if(!input.car.isSupersonic) {
            carPredictedTrajectory = new Parabola3D(
                    input.car.position,
                    input.car.velocity,
                    new Vector3(0, 0, -RlConstants.NORMAL_GRAVITY_STRENGTH).plus(naiveOrientationGuess.scaledToMagnitude(RlConstants.ACCELERATION_DUE_TO_BOOST_IN_AIR)),
                    0
            );
        }
        else {
            Vector3 acceleration = new Vector3(0, 0, -RlConstants.NORMAL_GRAVITY_STRENGTH).plus(naiveOrientationGuess.scaledToMagnitude(RlConstants.ACCELERATION_DUE_TO_BOOST_IN_AIR));
            carPredictedTrajectory = new Parabola3D(
                    input.car.position,
                    input.car.velocity,
    ...
    private double findTimeOfClosestApproachToBall(DataPacket input) {
        ...
        return Trajectory3D.findTimeOfClosestApproach(ballTrajectory, carPredictedTrajectory,
