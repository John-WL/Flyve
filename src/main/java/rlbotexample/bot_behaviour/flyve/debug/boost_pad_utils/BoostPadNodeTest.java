    ...
    public void updateGui(Renderer renderer, DataPacket input, double currentFps, double averageFps, long botExecutionTime) {
        ...
        Optional<BoostPad> closest = BoostManager.boostPads.stream().min(new Comparator<BoostPad>() {
            ...
            public int compare(BoostPad o1, BoostPad o2) {
                if(input.allCars.get(1-input.playerIndex).position.minus(o1.location).magnitudeSquared() <
                        input.allCars.get(1-input.playerIndex).position.minus(o2.location).magnitudeSquared()) {
                    return -1;
                }
                else if(input.allCars.get(1-input.playerIndex).position.minus(o1.location).magnitudeSquared() >
                        input.allCars.get(1-input.playerIndex).position.minus(o2.location).magnitudeSquared()) {
                    return 1;
                }
                return 0;
