package rlbotexample.bot.debug.rl_utils.circles;
...
public class RenderingCircle3DToTestIt extends FlyveBot {
    ...
    public void updateGui(Renderer renderer, DataPacket input, double currentFps, double averageFps, long botExecutionTime) {
        ...
        shapeRenderer.renderCross(circle.findClosestPointFrom(input.allCars.get(1-input.botIndex).position), Color.red);
        renderer.drawLine3d(Color.ORANGE, circle.findPointOnCircle(circle.findRadsFromClosestPoint(input.allCars.get(1-input.botIndex).position)).toFlatVector(), input.allCars.get(1-input.botIndex).position.toFlatVector());
    }
}
