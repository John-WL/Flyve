...
public class AlgorithmOfRotatorForOrientations extends FlyveBot {
    ...
    public BotOutput processInput(DataPacket input, GameTickPacket packet) {
        rotator = input.allCars.get(1-input.botIndex).orientation.findRotatorToRotateTo(orientation);
    ...
    public void updateGui(Renderer renderer, DataPacket input, double currentFps, double averageFps, long botExecutionTime) {
        ...
        renderer.drawLine3d(Color.red, input.allCars.get(1-input.botIndex).orientation.roofVector.rotate(rotator).scaled(300).plus(new Vector3(0, 0, 300)).toFlatVector(), new Vector3(0, 0, 300).toFlatVector());
